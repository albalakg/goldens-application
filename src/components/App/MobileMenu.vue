<template>
  <div class="mobile_menu_wrapper">
    <div class="mobile_menu_content">
        ...
        <template v-for="(link, index) in links">
          <div v-if="link.url" :key="index">
            <router-link class="simple_link" :to="link.url">
              <TabIcon :text="link.text" :icon="link.icon" :key="index" />
            </router-link>
          </div>
          <TabIcon
            v-else
      ...
      <search-dialog :show="showSearch" :minHeight="300" @close="close()" />
  ...
  computed: {
    links() {
      return this.allLinks.filter(
        (link) => link.logged === this.$store.getters["AuthState/isLogged"]
      );
    },
